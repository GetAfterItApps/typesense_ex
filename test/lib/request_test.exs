defmodule Typesense.RequestTest do
  ...
  alias Typesense.MockNode
  alias Typesense.MockPool
  alias Typesense.MockRequestConfig
  ...
  test "requests and handles a valid response" do
    ...
    assert %Typesense.Request{
             ...
             request_config: %Typesense.RequestConfig{
               api_key: "sdfsdf",
             ...
             request_params: %Typesense.RequestParams{
               method: :get,
  ...
  test "request retries, marks nodes unhealthy if they fail" do
    ...
    assert %Typesense.Request{response: {:maybe_retry, _request_params}} =
             response =
             Request.new()
             |> Request.execute(:get, "/fake-endpoint")
             |> Request.handle_response()
