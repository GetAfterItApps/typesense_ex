defmodule Typesense.Config do
  ...
  defp validate_api_key(%{errors: errors} = config) do
    %Config{config | errors: ["Configuration Missing API Key" | errors]}
  end
  ...
  defp validate_nearest_node(%{nearest_node: nearest_node_config, errors: errors} = config) do
    case node_errors(nearest_node_config) do
      ...
      node_errors ->
        %Config{config | errors: node_errors ++ errors}
    end
  end

  @spec validate_nodes(t()) :: t
  defp validate_nodes(%Config{nodes: nodes, errors: errors} = config) when nodes == [] do
    %Config{config | errors: ["Configuration Contains an Empty Node List" | errors]}
  end

  defp validate_nodes(%Config{nodes: nodes, errors: errors} = config) when not is_list(nodes) do
    msg = "Expected nodes to be a list but got '#{inspect(nodes)}'"
    %Config{config | errors: [msg | errors]}
  end

  defp validate_nodes(%Config{nodes: nodes, errors: errors} = config) do
    invalid_nodes =
      nodes
